"""
trainer.py — Trainer / Wrapper cho T-Learner (Criteo Uplift)
=============================================================
...
try:
    from .model import TLearnerModel
except ImportError:
    from baselines.tlearner.model import TLearnerModel
...
class TLEARNER:
    ...
    def fit(
        ...
        model_name: str = "tlearner",
        writer: Optional[Any] = None,
        verbose: int = 1,
    ) -> Dict[str, list]:
        """
        Huấn luyện mô hình T-Learner bằng DataLoaders truyền vào trực tiếp.
        ...
        if verbose:
            logger.info(
                f"Starting T-LEARNER training | Device: {self.device} | Epochs: {epochs}"
            )
