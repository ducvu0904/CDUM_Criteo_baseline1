"""
trainer.py — Trainer cho DRAGONNET (Criteo Uplift v2.1)
========================================================
...
try:
    from .model import DragonNetModel
except ImportError:
    from baselines.dragonnet.model import DragonNetModel
...
class DRAGONNET:
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for x_b, t_b, y_b in test_loader:
                ...
                loss = self.model.compute_loss(x_b_dev, t_b_dev, y_b_dev, alpha=self.alpha, beta=self.beta)
                total_loss += loss.item()
                n_batches += 1
    ...
    def fit(
        ...
        model_name: str = "dragonnet",
        writer: Optional[Any] = None,
        verbose: int = 1,
    ) -> Dict[str, list]:
        """Huấn luyện mô hình DRAGONNET bằng DataLoaders truyền vào trực tiếp."""
        ...
        if verbose:
            logger.info(
                f"Starting DRAGONNET training | Device: {self.device} | Epochs: {epochs}"
            )
    ...
    def predict_propensity(
        ...
        if isinstance(data, DataLoader):
            for batch in data:
                x_b = batch[0] if isinstance(batch, (list, tuple)) else batch
                x_b = x_b.to(self.device)
                prop = self.model.predict_propensity(x_b)
                prop_preds.append(prop.cpu().numpy())
        else:
            X_arr = _to_numpy(data)
            n_samples = len(X_arr)
            for offset in range(0, n_samples, batch_size):
                batch_x = torch.from_numpy(X_arr[offset : offset + batch_size]).to(self.device)
                prop = self.model.predict_propensity(batch_x)
                prop_preds.append(prop.cpu().numpy())

        return np.concatenate(prop_preds, axis=0)
